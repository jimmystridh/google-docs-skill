...
{
  "status": "error",
  "message": "File not found: abc123xyz",
...
3. Verify you have access to the file:
   ```bash
   scripts/drive_manager get-metadata \
     --file-id "FILE_ID"
   ```
...
{
  "status": "error",
  "message": "Insufficient permission to access file",
  "error_code": "API_ERROR"
}
```
...
{
  "status": "error",
  "message": "Invalid range: InvalidSheet!A1:B10",
  "error_code": "API_ERROR"
}
```
...
1. Get spreadsheet metadata to see available sheets:
   ...
   echo '{"spreadsheet_id":"SPREADSHEET_ID"}' | \
     scripts/sheets_manager get-metadata
   ```
...
{
  "status": "error",
  "message": "Index 5000 exceeds document length",
  "error_code": "API_ERROR"
}
```
...
{
  "status": "error",
  "message": "Request timeout: Network operation timed out",
  "error_code": "API_ERROR"
}
```
...
**Pattern**: `drive_manager [command] [--flags]`
...
2. Ensure proper quoting:
   ...
   echo '{"key": "$value"}' | sheets_manager read
...
**Solution**:
1. Apply number format via `format`:
   ```bash
   echo '{
     "spreadsheet_id": "ID",
     "sheet_id": 0,
...
1. **For Sheets**: Use specific ranges instead of entire sheets:
   ```bash
   # SLOW ❌
   echo '{"spreadsheet_id":"ID","range":"Sheet1"}' | sheets_manager read

   # FAST ✅
   echo '{"spreadsheet_id":"ID","range":"Sheet1!A1:Z1000"}' | sheets_manager read
   ```

2. **For Docs**: Read structure first, then specific sections:
   ```bash
   # Get structure to understand document
   docs_manager structure "DOC_ID"
...
3. **For Drive**: Limit search results:
   ```bash
   drive_manager search --query "name contains 'Report'" --max-results 50
   ```
...
```bash
drive_manager list > output.json
cat output.json | jq .
```
...
```bash
drive_manager get-metadata --file-id "FILE_ID"
```
...
### Check Build/Runtime Setup

Rust/Cargo errors may provide additional context:
```bash
# Verify project compiles
cargo check --offline

# Run manager help to verify binaries resolve
...
- Review Rust source code for implementation details
